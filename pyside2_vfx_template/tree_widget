...
def filter_tree(
    ...
    """Filter tree items based on input from a filter bar.

    This function filters the items of a tree by displaying or hiding them based
    ...
    for i in range(root.childCount()):
        ...
        if item.childCount() > 0:
            should_hide_parent = all(
                filter_text not in item.child(j).text(column).lower()
